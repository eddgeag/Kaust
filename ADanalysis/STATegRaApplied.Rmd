...
knitr::opts_chunk$set(comment = NA, prompt = TRUE, tidy.opts=list(width.cutoff=60),tidy=TRUE, 
               ...
               message = FALSE, warning = FALSE, cache=T,fig.pos = "!H", out.extra = "")
```
...
for ( i in 1:dim(metadata.copy)[2]) {
  if (class(metadata.copy[,i])!="factor"){
      ...
      lines(density(metadata.copy[,i]),color="blue")
...
title.temp<-matrix(strsplit2(temp,"-"),ncol=3)
...
```{r investigando}
boxplot(targets[,c(4,8,9,12)])
```

```{r investigando2}
targets.temp2<-targets.temp[,-c(3)]
rownames(targets.temp2)<-targets$shortname
