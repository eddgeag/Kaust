...
levels(CLL_metadata.copy$IGHV)<-c(F,T)
levels(CLL_metadata.copy$trisomy12)<-c(F,T)
...
library(papeR)
CLL_metadata.descriptive<-CLL_metadata.survival
library("knitr")
library(kableExtra)
...
s1<-summarize(CLL_metadata.descriptive,type="factor",group="treatedAfter",cumulative=F,test=F)
s2<-summarize(CLL_metadata.descriptive, type = c("numeric"), variables=names(CLL_metadata.descriptive),group = "treatedAfter", cumulative = F,test=F)
colsnames<-colnames(s1)
colnames(s1)<-c(" " ," ","No Treatment"," "," ","Treatment"," "," ")
micolumna<-data.frame(" "=colsnames[1],
                      " "=colsnames[2],
                      "No Treatment" = colsnames[3],
                      " "= colsnames[4],
                      " " = colsnames[5],
                      "Treatment" = colsnames[6],
                      " "=colsnames[7],
                      " "=colsnames[8])
kableExtra::kable(as.data.frame(rbind(colsnames,s1)))
                      
kableExtra::kable(s2)
# kable(summarize(CLL_metadata.shared, type = "numeric", group = "treatedAfter", test = FALSE))
```

Regarding the TTT and TTD, are two variables that measure time, since the extraction of the sample untill next tratment, and TTD is the time to death since the sample was taken. Thus, that's why, when there is no treatment, both values are the same.

## Survival Analysis
```{r}
CLL_metadata.survival$survivalOutcome<-Surv(time=CLL_metadata.survival$TTD,
                                     event=CLL_metadata.survival$died==F)
...
CLL_metadata.survival$survivalTreatment<-Surv(time=CLL_metadata.survival$TTT,
                                     event=CLL_metadata.survival$treatedAfter==F)
...
It is odd that when we plot the Kaplan Meier curves, when the event equals to death, the kapplan meier, de dots, are the people alive. The same thing, it's like it takes the opposite argument.
...
```{r}
treat<-sumarizacion(CLL_metadata.copy,"tratamiento",T)
...
```{r}
NOtreat<-sumarizacion(CLL_metadata.copy,"tratamiento",F)
...
There is no difference between TTT and TTD when there is no tratment, because its the same time.

## By trisomy

```{r}
tri<-sumarizacion(CLL_metadata.copy,"trisomia",T)
...
```{r}
no.tri<-sumarizacion(CLL_metadata.copy,"trisomia",F)
...
## By mutation
```{r}
mut<-sumarizacion(CLL_metadata.copy,"mutacion",T)
...
```{r}
NOmut<-sumarizacion(CLL_metadata.copy,"mutacion",F)
  ...
  for(i in 1:ncol(clinical.variables)){
    ...
    for(j in 1:ncol(clinical.variables)){
      ...
      if((class(clinical.variables[,i])=="factor") && (class(clinical.variables[,j])=="factor")){
        ...
        if(colnames(clinical.variables)[i]=="survivalTreatment"){
          
          if(colnames(clinical.variables)[j]=="died"){
            
            pvalues[i,j]<-NA
            
          }else if(colnames(clinical.variables)[j]=="treatedAfter"){ 
            pvalues[i,j]<-NA
          }else{
            pvalues[i,j]<-summary(coxph(clinical.variables[,i]~clinical.variables[,j]))$coefficients[5]
       ...
       if(colnames(clinical.variables)[j]=="survivalTreatment"){
          
          if(colnames(clinical.variables)[i]=="died"){
            
            pvalues[i,j]<-NA
            
          }else if(colnames(clinical.variables)[i]=="treatedAfter"){ 
            pvalues[i,j]<-NA
          }else{
            pvalues[i,j]<-summary(coxph(clinical.variables[,j]~clinical.variables[,i]))$coefficients[5]
          ...
          if(colnames(clinical.variables)[j]=="survivalOutcome"){
...
```{r}
minfi::densityPlot(mat.met,xlab="M")
```
...
s1<-summarize(CLL_metadata.shared,type="factor",group="treatedAfter",cumulative=F,test=F)
colsnames<-colnames(s1)
colnames(s1)<-c(" " ," ","No Treatment"," "," ","Treatment"," "," ")
micolumna<-data.frame(" "=colsnames[1],
                      " "=colsnames[2],
                      ...
                      " "=colsnames[8])
kableExtra::kable(as.data.frame(rbind(colsnames,s1)))
kable(summarize(CLL_metadata.shared, type = c("numeric","factor"), variables=names(CLL_metadata.shared),group = "treatedAfter", cumulative = F,test=F))
# kable(summarize(CLL_metadata.shared, type = "numeric", group = "treatedAfter", test = FALSE))
```
...
Before start the multi-omic approach, mean centred of each feature and the data set normalization to unit sum of squares (forbenious normalization) is recommended to make more comparabable the different omics data-sets.

```{r}
#Calculate Forbenious normalization
frobenius_rna <- norm(as.matrix(v$E), type="F")
frobenius_met <- norm(as.matrix(mat.met), type="F")

#Mean-centring and division by Frobenius normalization factor
rna_norm <- t(scale(t(v$E), scale=FALSE))/frobenius_rna
met_norm <- t(scale(t(mat.met), scale=FALSE))/frobenius_met

##Mean-centring without Frobenius normalization
rna_norm2 <- t(scale(t(v$E), scale=FALSE))
met_norm2 <- t(scale(t(mat.met), scale=FALSE))
```

Then, we need to keep the common samples between the different data-types.

```{r}
ids_rna_met <- intersect(colnames(v$E), colnames(mat.met))
length(ids_rna_met)
```
...
# Data <- list(mRNA=v$E[,ids_rna_met],
#               met=mat.met[,ids_rna_met])
# rjive_rna_met <- jive(Data, scale = FALSE, center = FALSE)
load(file="./results/rjiveData.RData")
```

```{r}
showVarExplained(rjive_rna_met) 
```
...
```{r}
sharedComponets<-function(result,n_joint){
  
   PCs = matrix(nrow = n_joint, ncol = dim(result$data[[1]])[2])
...
individualComponents<-function(result,n_indiv){
    
   PCs_2 = matrix(nrow = n_indiv, ncol = dim(result$data[[1]])[2])
   PCs_1 = matrix(nrow = n_indiv, ncol = dim(result$data[[1]])[2])
   SVD_1 = svd(result$individual[[1]], nu = n_indiv, 
      nv = n_indiv)
   SVD_2 = svd(result$individual[[2]], nu = n_indiv, 
      nv = n_indiv)
   
   PCs_1[1:n_indiv,] = diag(SVD_1$d)[1:n_indiv,1:n_indiv] %*% 
     t(SVD_1$v[,1:n_indiv])
   PCs_2[1:n_indiv,] = diag(SVD_2$d)[1:n_indiv,1:n_indiv] %*% 
     t(SVD_2$v[,1:n_indiv])

    # PC_names[1:n_joint] = paste("Joint ", 1:n_joint)
    lista<-list(rna=PCs_1,dnamet = PCs_2)
    return(lista)
...
```{r}
shared<-sharedComponets(rjive_rna_met,101)
individual<-individualComponents(rjive_rna_met,101)
```

```{r}
individual$rna[1,]-individual$dnamet[1,]
```


```{r}
colores<-as.numeric(CLL_metadata.shared$IGHV)
showPCA(rjive_rna_met,n_joint=2,Colors=colores,n_indiv=c(1,1))
```
